...
public:
  ...
  /// batchSize 文字分を目安に辞書から文字を確保し、二次元配列に整形して返す。
  /// row × column が batchSize の倍数でない場合は std::invalid_argument を送出する。
  /// </summary>
  /// <param name="row">生成する行数。1 以上であること。</param>
  /// <param name="column">生成する列数。1 以上であること。</param>
  /// <param name="batchSize">候補抽出時に確保したい最小文字数。辞書語を順に積み上げて到達させる。</param>
